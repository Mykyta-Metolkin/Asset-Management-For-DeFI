...
def build_indexes(aggregator):
    ...
    summary_df['CALMAR'] = summary_df['performance']/abs(summary_df['max_dd'])
    summary_df['avg_delta'] = summary_df['price_rel_delta'].mean()
    summary_df = summary_df[['snapped_at','folder','performance','avg_delta','rel_delta_stdev', 'dd', 'max_dd', 'CALMAR']]
...
def create_corr_df(df):
    ...
    df = df[['snapped_at', 'folder', 'performance']]
    dates = df['snapped_at'].unique()
    indexes = df['folder'].unique()
    df_temp = pd.DataFrame()
    df_n = pd.DataFrame()
    for idx in indexes:    
        df_temp = df[df['folder'] == idx]  
...
def build_histogram(corr_df, output_path):
    ...
    heatmap.set_title('Indexes Correlation Heatmap', fontdict={'fontsize':14}, pad=14)
    plt.savefig(output_path+'heatmap.png', dpi=500)
...
def SelloffRally(selloff_rally):
    ...
    indexes_corr = create_corr_df(aggregated_data_list[1])
    build_histogram(indexes_corr, output_path)
